...
connection.on("ReceiveMessage", function (message) {
    ...
    document.getElementById("mymessagesList").appendChild(li);
});

document.getElementById("usermsg").addEventListener("click", async (event) => {
    var username = document.getElementById("user-name").value;
    var usermessagetext = document.getElementById("user-message-text").value;

    try {
        await connection.invoke("SendPrivateMessage", username, usermessagetext);
    }
    catch (e) {
